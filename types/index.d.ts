...
interface InstanceProps<S> {
  idSelector: Selector<S, string | number>;
}
type OutputInstanceProps<S, R, C> = OutputProps<C> & {
  use: (id: string | number) => () => void;
...
interface ParametricInstanceProps<S, P> {
  idSelector: ParametricSelector<S, P, string | number>;
}

export type OutputSelector<S, R, C> = Selector<S, R> & OutputProps<C>;

export type InstanceSelector<S, R> = Selector<S, R> & InstanceProps<S>;
export type OutputInstanceSelector<S, R, C> = Selector<S, R> &
  OutputProps<C> &
  OutputInstanceProps<S, R, C> &
  InstanceProps<S>;

export type OutputParametricSelector<S, P, R, C> = ParametricSelector<S, P, R> &
  OutputProps<C>;
export type ParametricInstanceSelector<S, P, R> = ParametricSelector<S, P, R> &
  ParametricInstanceProps<S, P>;
export type OutputParametricInstanceSelector<S, P, R, C> = ParametricSelector<
  S,
  P,
  R
> &
  OutputParametricInstanceProps<S, P, R, C> &
  ParametricInstanceProps<S, P>;

export function isInstanceSelector<S, R, C>(
  selector: OutputSelector<S, R, C>
): selector is OutputInstanceSelector<S, R, C>;
export function isInstanceSelector<S, P, R, C>(
  selector: OutputParametricSelector<S, P, R, C>
): selector is OutputParametricInstanceSelector<S, P, R, C>;
...
interface IdSelector<P> {
  (props: P, ...args: any[]): string | number;
}

export function createIdSelector<P>(
  idSelector: IdSelector<P>
): ParametricInstanceSelector<{}, P, string | number>;
